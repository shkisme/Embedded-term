...
void GPIO_Configure(void)
{
    ...
    GPIO_Init(GPIOC, &GPIO_InitStructure);
     ...
     // 전원이 정상적으로 인가되었는지 확인
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2 | GPIO_Pin_13 | GPIO_Pin_15; // Enable Motor : PD13, PD15(나중에 OR을 통해 2개까지 조작 가능)
        ...
        if(word == '1'){ // 자동 환기 시스템 작동 시작
          ...
          message_index = 1;
            ...
            if (isOpen == false){  // 닫혀있을때
                ...
                //
                isSpin = true;
                GPIO_ResetBits(GPIOD, GPIO_Pin_2 | GPIO_Pin_13 | GPIO_Pin_15);   // Motor Off
                //
            ...
            if (isOpen == true){  // 열려있을때
                ...
                //
                isSpin = false;
                GPIO_SetBits(GPIOD, GPIO_Pin_2 | GPIO_Pin_13 | GPIO_Pin_15);   // Motor Off
                //
                timer_status = NO_TIMER; // 타이머 초기화
                current_time = 0;        // 타이머 초기화
                message_index = 6;
    ...
    if(EXTI_GetITStatus(EXTI_Line4) != RESET) { // KEY1
        if(GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_4) == Bit_RESET) {
          ...
          if(isSpin == true){
            ...
            //GPIO_SetBits(GPIOD, GPIO_Pin_2 | GPIO_Pin_13 | GPIO_Pin_15);   // Motor On
            isSpin = true;
            }
        }
...
void startVen(){ // 환기 시작
    ...
    isLed = true;
    // TODO : 모터 작동 함수 호출
    startMotor();    LED_On();    // TODO : LED 빨간불 켜기 함수 호출
}
