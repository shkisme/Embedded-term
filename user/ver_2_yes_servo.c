...
void Servo_posimove(int Init_duty, int Direction, int delay){ // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      for (int dutyCycle = Init_duty; dutyCycle <= 1500; dutyCycle += Direction)
    {
        ...
        delay_ms(40*delay);  // motor 돌아가는 시간
    }
...
void USART2_IRQHandler() { // 폰에서 값 받아옴
    ...
    if(USART_GetITStatus(USART2,USART_IT_RXNE) != RESET){
        ...
        } else if (word == '5' && isStart == 1){ // 수동 환기 시작
...
void ADC1_2_IRQHandler(void) {  // 아날로그 센서값을 받아옴 @wlqrkrhtlvek
  ...
  if(ADC_GetITStatus(ADC2,ADC_IT_EOC) != RESET) {
    ...
    if ((int)analogData_2 <= LIGHT_FLAG && enableTimer == true && timer_status == NO_TIMER){ // 연기 상태 나쁠 때 /////@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        startVen();
    }
  }
...
int main(void)
{
    ...
    GPIO_ResetBits(GPIOD, GPIO_Pin_14);  // LED
    // --------    
    LCD_Init();
    ...
    while(true){
        ...
        while (isStart == true) {
            ...
            if( isWindowOpening == true && isWindowClosing == false && isClosed == true){ // open
              ...
              Servo_negamove(2500, 100, 1); // open
              isClosed = true;
            ...
            LCD_ShowNum(80, 260, (int)analogData_1, 4, BLACK, WHITE);
