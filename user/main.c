...
int SMOKE_FLAG = 1600; // 연기 농도가 높은지 판단하는 값
int LIGHT_FLAG = 2900; // 밝기 농도 판단

uint16_t analogData_1, analogData_2;
...
void EXTI4_IRQHandler(void){
    if(EXTI_GetITStatus(EXTI_Line4) != RESET) { // KEY1
        if(GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_4) == Bit_RESET) {
          ...
          if(isSpin == true){
                GPIO_ResetBits(GPIOD, GPIO_Pin_14);   // Motor Off
                 Servo_negamove(2500, 100, 5);
                isSpin = false;
            } else {
                GPIO_SetBits(GPIOD, GPIO_Pin_14);   // Motor On
...
void ADC1_2_IRQHandler(void) {  // 아날로그 센서값을 받아옴 @wlqrkrhtlvek
  ...
  if(ADC_GetITStatus(ADC1,ADC_IT_EOC) != RESET) {
    ...
    if (analogData_1 <= SMOKE_FLAG && timer_status == NO_TIMER){ // 연기 상태 나쁠 때
        startVen();
    }
  }
...
void startMotor(){ // DC 모터 작동
    isSpin = true;
    timer_status = TWENTY_SEC;
    current_time = 0;
    Servo_negamove(2500, 100, 5);
    GPIO_ResetBits(GPIOD, GPIO_Pin_12 | GPIO_Pin_14);   // Motor On
}
...
void stopMotor(){ // 모터 중지
    ...
    GPIO_SetBits(GPIOD, GPIO_Pin_12 | GPIO_Pin_14);   // Motor Off
}
 ...
 int main(void)
{
    ...
    GPIO_SetBits(GPIOD, GPIO_Pin_12);  // Motor
    GPIO_SetBits(GPIOD, GPIO_Pin_14);  // LED
    // --------    
    LCD_Init();
